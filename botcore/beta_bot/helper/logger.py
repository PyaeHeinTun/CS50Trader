...
def logger(trade_list: dict[str,list[Trade]], startTime: datetime, endTime: datetime):
...
def logger_test(future_predictions: dict, startTime: datetime, endTime: datetime):
    ...
    table.field_names = ["symbol"] + minute_list + ["trend"] + ["ms"]
    table.float_format = f'.{6}'
    ...
    for symbol in symbol_list:
        ...
        trend = next(iter(coin_data.values()))['trend']
        table.add_row(
            [symbol] + class_prediction + [int(trend)] + [round((endTime-startTime).microseconds * 0.001, 2)]
            )
