...
def get_signal(dataframe:pd.DataFrame,symbol:str) -> tuple:
    ...
    dataframe['trend'] = adaptiveTrendFinder_jit(dataframe,[10,20,30,40])
    predicted_class = CoreML(symbol).train(dataframe,config['training_params'])
    ...
    if (dataframe['trend'].iloc[-1] > 0):
        ...
        current_trend = -1
    return current_trend , predicted_class